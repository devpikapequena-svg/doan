...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("Request body received:", requestBody) // Log do corpo da requisição
    ...
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      console.log("Invalid amount:", amount) // Log do erro de valor inválido
      return NextResponse.json({ error: 'Valor do pagamento inválido.' }, { status: 400 })
    }
    const amountInCents = Math.round(parsedAmount * 100)

    if (!Array.isArray(items) || items.length === 0) {
      console.log("Invalid items:", items) // Log de itens inválidos
    ...
    console.log("Payload for BuckPay:", payloadForBuckPay) // Log do payload enviado
    ...
    console.log("BuckPay Response:", responseData) // Log da resposta da BuckPay

    if (!buckpayResponse.ok) {
      console.log("BuckPay error:", responseData.error) // Log de erro da BuckPay
    ...
    if (buckpayData?.id) {
      ...
      try {
        console.log("Sending to Utmify:", utmifyPayload) // Log antes de enviar para Utmify
        await sendOrderToUtmify(utmifyPayload)
      } catch (utmifyError: any) {
        console.error(`Erro ao enviar pedido pendente ${buckpayData.id} para Utmify:`, utmifyError)
      }
    }

    return NextResponse.json(responseData, { status: 200 })
  } catch (error: any) {
    console.error('[create-payment POST] Erro fatal:', error) // Log do erro geral
    return NextResponse.json(
